...
public class CinderboltBeamEntityRenderer extends EntityRenderer<CinderboltBeamEntity, CinderboltBeamEntityRenderer.BeamRenderState> {
        ...
        for (int p = 0; p < BOLT_PAIRS; p++) {
            ...
            snc.submitCustomGeometry(ps, RenderTypes.lightning(), (pose, v) ->
                    drawBolt(pose, v, from, dir, right, up, len,
                            0.08f, 255, 120, 0, 230, age, seedA));
            snc.submitCustomGeometry(ps, RenderTypes.lightning(), (pose, v) ->
                    drawBolt(pose, v, from, dir, right, up, len,
                            0.05f, 255, 220, 30, 200, age, seedB));
        }
    }
    ...
    private static void drawImpactBolts(BeamRenderState s, PoseStack ps, SubmitNodeCollector snc) {
        ...
        for (int slot = 0; slot < IMP_NUM_SPARKS; slot++) {
            ...
            for (int seg = 0; seg < IMP_NUM_SEGS; seg++) {
                ...
                snc.submitCustomGeometry(ps, RenderTypes.lightning(), (pose, v) -> {
                    crossQuad(pose, v, a, b, tA, 0.07f, 255, 140, 0, al);
                    crossQuad(pose, v, a, b, tB, 0.07f, 255, 140, 0, al);
                });
            }
        }
    }
