...
public class PolarityArrowRedEntityRenderer extends ArrowRenderer<PolarityArrowRedEntity, PolarityArrowRenderState> {

    private static final Identifier TEXTURE =
            Identifier.fromNamespaceAndPath("rituals",
                    "textures/entity/projectiles/polarity_arrow_red.png");

    public PolarityArrowRedEntityRenderer(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @Override
    public void extractRenderState(PolarityArrowRedEntity entity, PolarityArrowRenderState state, float partialTick) {
        ...
        for (int i = 0; i < size; i++) {
            int idx = Math.floorMod(entity.trailHead - 1 - i, PolarityArrowRedEntity.TRAIL_LENGTH);
    ...
    private static void drawTrail(PolarityArrowRenderState state, PoseStack ps, SubmitNodeCollector snc) {
        ...
        for (int i = 0; i < totalSegs; i++) {
            ...
            // Using RGB (220, 20, 50) for a vibrant red/crimson trail.
            snc.submitCustomGeometry(ps, RenderTypes.lightning(), (pose, v) -> {
                v.addVertex(pose, (float)fa0.x, (float)fa0.y, (float)fa0.z).setColor(220, 20, 50, iaA);
                v.addVertex(pose, (float)fa1.x, (float)fa1.y, (float)fa1.z).setColor(220, 20, 50, iaA);
                v.addVertex(pose, (float)fb1.x, (float)fb1.y, (float)fb1.z).setColor(220, 20, 50, iaB);
                v.addVertex(pose, (float)fb0.x, (float)fb0.y, (float)fb0.z).setColor(220, 20, 50, iaB);
