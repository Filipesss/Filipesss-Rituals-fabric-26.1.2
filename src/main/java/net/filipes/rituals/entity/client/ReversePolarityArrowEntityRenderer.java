...
public class ReversePolarityArrowEntityRenderer extends ArrowRenderer<ReversePolarityArrowEntity, PolarityArrowRenderState> {

    private static final Identifier TEXTURE =
            Identifier.fromNamespaceAndPath("rituals",
                    "textures/entity/projectiles/reverse_polarity_arrow.png");

    public ReversePolarityArrowEntityRenderer(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @Override
    public void extractRenderState(ReversePolarityArrowEntity entity, PolarityArrowRenderState state, float partialTick) {
        ...
        for (int i = 0; i < size; i++) {
            int idx = Math.floorMod(entity.trailHead - 1 - i, ReversePolarityArrowEntity.TRAIL_LENGTH);
    ...
    @Override
    public Identifier getTextureLocation(PolarityArrowRenderState state) { return TEXTURE; }
    ...
    private static void drawTrail(PolarityArrowRenderState state, PoseStack ps, SubmitNodeCollector snc) {
        ...
        for (int i = 0; i < totalSegs; i++) {
            Vec3 a = smooth.get(i), b = smooth.get(i + 1);
            float tA = i / (float) totalSegs, tB = (i + 1) / (float) totalSegs;
            float alphaA = 1.0f - tA, alphaB = 1.0f - tB;
            ...
            final int iaA = (int)(alphaA * 200), iaB = (int)(alphaB * 200);
            final Vec3 fa0=a0, fa1=a1, fb0=b0, fb1=b1;

            snc.submitCustomGeometry(ps, RenderTypes.lightning(), (pose, v) -> {
                v.addVertex(pose, (float)fa0.x, (float)fa0.y, (float)fa0.z).setColor(255, 220, 30, iaA);
                v.addVertex(pose, (float)fa1.x, (float)fa1.y, (float)fa1.z).setColor(255, 220, 30, iaA);
                v.addVertex(pose, (float)fb1.x, (float)fb1.y, (float)fb1.z).setColor(255, 220, 30, iaB);
                v.addVertex(pose, (float)fb0.x, (float)fb0.y, (float)fb0.z).setColor(255, 220, 30, iaB);
    ...
    private static List<Vec3> catmullRomSubdivide(List<Vec3> pts, int steps) {
        ...
        for (int i = 1; i < padded.size() - 2; i++) {
            Vec3 p0=padded.get(i-1), p1=padded.get(i), p2=padded.get(i+1), p3=padded.get(i+2);
            out.add(p1);
            for (int j = 1; j < steps; j++) out.add(catmullRom(p0, p1, p2, p3, j / (float) steps));
        }
