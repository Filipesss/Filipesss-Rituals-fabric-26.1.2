...
import net.filipes.rituals.entity.custom.WindGustBigEntity;
...
public class WindGustBigEntityRenderer
        extends EntityRenderer<WindGustBigEntity, WindGustBigEntityRenderer.TrailRenderState> {
    ...
    private static final RenderType[] RENDER_TYPES = new RenderType[WindGustBigEntity.FRAME_COUNT];
    static {
        for (int i = 0; i < WindGustBigEntity.FRAME_COUNT; i++) {
            Identifier tex = Identifier.fromNamespaceAndPath(
                    "rituals", "textures/particle/wind_gust_" + i + ".png");
            RENDER_TYPES[i] = RenderTypes.entityTranslucentEmissive(tex);
        }
    }

    public WindGustBigEntityRenderer(EntityRendererProvider.Context ctx) { super(ctx); }

    @Override public TrailRenderState createRenderState() { return new TrailRenderState(); }

    @Override
    public void extractRenderState(WindGustBigEntity e, TrailRenderState s, float pt) {
    ...
    @Override public boolean affectedByCulling(WindGustBigEntity e) { return false; }
    ...
    public void submit(TrailRenderState s, PoseStack ps,
        ...
        final float half = WindGustBigEntity.QUAD_SIZE * 0.5f;
